...
function open_gallery(id) {
    ...
    $.ajax({
        url: window.baseURL + 'products/ajax-get_image/' + id,
        type: "GET",
        dataType: "JSON",
        success: function (data) { // $(data.data).each(function (key, val) {
            ...
            for (i = 0; i < data.length; i++) {
                ...
                $('<div class="item"><img src="' + window.baseURL + data[i]["ThumbNail"] + '" alt="Product id #' + data[i]["id"] + '"><div class="carousel-caption"><h3>Product ID #' + data[i]["id"] + '</h3><p>' + data[i]["Description"] + '</p></div></div>').appendTo('.carousel-inner');
...
$(function () { // $('#start_date').daterangepicker({singleDatePicker: true});
    ...
    $('a[data-toggle="tab"]').on('shown.bs.tab', function (e) {
        ...
        if ((target == '#localproductstep3')) {
            $.ajax({
                url: window.baseURL + 'products/ajax-calculating-summary',
                type: "GET",
                dataType: "JSON",
...
function add_localproduct() {
    ...
    $("#flnFile").fileinput('refresh', {
        ...
        defaultPreviewContent: '<img id="imgfile" class="rounded-circle" src="' + window.baseURL + 'assets/img/avatars/avatar.png" alt="Product Picture" width="80px" height="80px">',
        layoutTemplates: {
...
function get_boom_table(id) {
    ...
    $.ajax({
        url: window.baseURL + 'products/ajax-edit/' + id,
        type: "GET",
        dataType: "JSON",
...
function edit_product(id, flag) {
    ...
    if ($("#indus_type").val() == "Cosmetics") {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $("#flnFile").fileinput('refresh', {
                    ...
                    elErrorContainer: '#kv-avatar-errors-1',
                    msgErrorClass: 'alert alert-block alert-danger',
                    defaultPreviewContent: '<img id="imgfile" class="rounded-circle" src="' + window.baseURL + data[0].ThumbNail + '" alt="Product Picture" width="80px" height="80px">',
                    layoutTemplates: {
        ...
        // Ajax Load data from ajax
        $.ajax({
            url: window.baseURL + 'imported-products/ajax-editimported/' + id,
            type: "GET",
            dataType: "JSON",
            success: function (data) {
...
function delete_sale(id, row) {
    ...
    $("#btnYesConfirmYesNo").off('click').click(function () {
        $.ajax({
            url: window.baseURL + 'products/ajax-delete_sale/' + id,
            type: "POST",
            dataType: "JSON",
            row: row,
            success: function (data) {
...
function delete_bom(id, row) {
    ...
    $("#btnYesConfirmYesNo").off('click').click(function () {
        $.ajax({
            url: window.baseURL + 'products/ajax-delete_bom/' + id,
            type: "POST",
            dataType: "JSON",
            row: row,
            success: function (data) {
...
function delete_import_bom(id, row) {
    ...
    if (confirm('Are you sure you want to delete this data?')) { // ajax delete data to database
        $.ajax({
            url: window.baseURL + 'imported-products/ajax-delete_import_bom/' + id,
            type: "POST",
            dataType: "JSON",
            row: row,
            success: function (data) {
...
function saveTable() {
    ...
    if ($('[name="status"]').val() == "update") {
        url = window.baseURL + 'products/ajax-update/' + counter;
    } else {
        url = window.baseURL + 'products/ajax-add/' + counter;
    }

    if ($('#flnFile').val() == "" && $('#imgfile').attr('src') == window.baseURL + "assets/img/avatars/avatar.png") {

        $('#modalConfirmYesNo').modal('show');

        $("#lblTitleConfirmYesNo").html("User Confirmation");
        $("#lblMsgConfirmYesNo").html("<div class='col-sm-3'>" + "<img id='imgfile' class='rounded-circle' src='" + window.baseURL + "assets/images/default-avatar.png' alt='Product Picture' width='80px' height='80px'>" + "</div>" + "<div class='col-sm-9'>" + "If you want to add image <strong>Click on folder icon <i class='fa fa-folder-o'></i>, then select an image <i class='fa fa-file-image-o'></i>.</strong><br/>Do you want to add image now ??" + "</div>");
    ...
    if ($("#indus_type").val() != "Cosmetics") {
        if ($('[name="status"]').val() == "update") {
            if ($('[name="methode"]').val() == 1) {
                ...
                //row counter starts from 0 so -1 from current rows
                counter = current_rows - 1;
                url = window.baseURL + 'imported-products/ajax-updateimported/' + counter;
            } else if ($('[name="methode"]').val() == 0) {
                var current_rows = $("#table_bom > tbody").children().length;
                var current_rows_sales = $("#tab_logic > tbody").children().length;
                //row counter starts from 0 so -1 from current rows
                counter = current_rows - 1;
                url = window.baseURL + 'products/ajax-update/' + counter + '/' + current_rows_sales;
            }
        } else {
            if ($("input[name='revenue']:checked").val() == 'import') {
                url = window.baseURL + 'imported-products/ajax-addimported/' + counter;
        ...
        if ($('[name="status"]').val() == "update") {
            url = window.baseURL + 'products/ajax-update';
        } else {
            url = window.baseURL + 'products/ajax-add';
        }
...
function delete_localproduct(id) {
    $.ajax({
        url: window.baseURL + 'products/ajax-delete/' + id,
        type: "POST",
        dataType: "JSON",
...
function delete_importproduct(id) {
    $.ajax({
        url: window.baseURL + 'imported-products/ajax-deleteimported/' + id,
        type: "POST",
        dataType: "JSON",
        success: function (data) { // if success reload ajax table
            $('#table_form').modal('hide');
...
function delete_cosmeticproduct(id) {
    $.ajax({
        url: window.baseURL + 'products/ajax-delete-cosmetics-products/' + id,
        type: "POST",
        dataType: "JSON",
        success: function (data) { // if success reload ajax table
            $('#table_form').modal('hide');
