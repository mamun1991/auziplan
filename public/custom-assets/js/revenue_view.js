...
$(function () {
    ...
    if (count == 0) {
        ...
        $('#revenue_form').find('input[name="max_view"]').each(function (e) {
        ...
        $('#revenue_form').find('input[name="no_purchase"]').each(function (e) {

            $(this).val(number_format($(this).val(), 0, '.', ','))
        })
        $('#revenue_form').find('input[name="no_reorder"]').each(function (e) {
        ...
        $('#revenue_form').find('input[name="product_price"]').each(function (e) {
...
// Added Rule For Greater Than 0 Value in jquery
    ...
    $.ajax({
        ...
        success: function (data) { // console.log(data);return false;
            if (data.status) {
                ...
                $('#modalConfirmYesNo').modal('show');
                $("#lblTitleConfirmYesNo").html("");
                $("#lblMsgConfirmYesNo").html("<div class='checkmark'>" + "<svg version='1.1' id='Layer_1' xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink' x='0px' y='0px' viewBox='0 0 161.2 161.2' enable-background='new 0 0 161.2 161.2' xml:space='preserve'>" + "<path class='path' fill='none' stroke='#7DB0D5' stroke-miterlimit='10' d='M425.9,52.1L425.9,52.1c-2.2-2.6-6-2.6-8.3-0.1l-42.7,46.2l-14.3-16.4" + "c-2.3-2.7-6.2-2.7-8.6-0.1c-1.9,2.1-2,5.6-0.1,7.7l17.6,20.3c0.2,0.3,0.4,0.6,0.6,0.9c1.8,2,4.4,2.5,6.6,1.4c0.7-0.3,1.4-0.8,2-1.5" + "c0.3-0.3,0.5-0.6,0.7-0.9l46.3-50.1C427.7,57.5,427.7,54.2,425.9,52.1z'/>" + "<circle class='path' fill='none' stroke='#7DB0D5' stroke-width='4' stroke-miterlimit='10' cx='80.6' cy='80.6' r='62.1'/>" + "<polyline class='path' fill='none' stroke='#28B463' stroke-width='6' stroke-linecap='round' stroke-miterlimit='10' points='113,52.8,74.1,108.4 48.2,86.4 '/>" + "<circle class='spin' fill='none' stroke='#7DB0D5' stroke-width='4' stroke-miterlimit='10' stroke-dasharray='12.2175,12.2175' cx='80.6' cy='80.6' r='73.9'/>" + "</svg>" + "</div>" + "<p class='alert-box'>Your Sales have been delete successfully !!!</p>");
...
$(".wizard").each(function () {
    ...
    $(wizard).append('<div class="footer"><button type="button" id="btnFormPrev" class="btn btn">Prev</button><button id="btnFormNext" type="button" class="btn btn">Next</button></div>');
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.status) {
                ...
                drawBarChart($('#potential_cust_yearly_1').html().replace(/,/g, '').replace(/\$|\€|\₹|\£\%/g, '').replace(/%/g, ''), $('#potential_cust_yearly_2').html().replace(/,/g, '').replace(/\$|\€|\₹|\£\%/g, '').replace(/%/g, ''), $('#potential_cust_yearly_3').html().replace(/,/g, '').replace(/\$|\€|\₹|\£\%/g, '').replace(/%/g, ''), cur);
            } else {
...
$('.clickable-row').on('click', function (event) {
    ...
    drawBarChart($(this).children('td:eq(1)').text().replace(/,/g, '').replace(/\$|\€|\₹|\£\%/g, '').replace(/%/g, ''), $(this).children('td:eq(2)').text().replace(/,/g, '').replace(/\$|\€|\₹|\£\%/g, '').replace(/%/g, ''), $(this).children('td:eq(3)').text().replace(/,/g, '').replace(/\$|\€|\₹|\£\%/g, '').replace(/%/g, ''), cur);
});
...
function calculationRevenue_summary(cur, ser) {
    ...
    if (parseFloat($('#ex1').val()) == 0) {
        ex1_value = 1
    } else {
        ex1_value = $('#ex1').val();
    }
    if (parseFloat($('#ex2').val()) == 0) {
        ex2_value = 1
    } else {
        ex2_value = $('#ex2').val();
    }
    if (parseFloat($('#ex3').val()) == 0) {
        ex3_value = 1
    } else {
        ex3_value = $('#ex3').val();
    }
    if (parseFloat($('#ex4').val()) == 0) {
        ex4_value = 1
    } else {
        ex4_value = $('#ex4').val();
    }
