...
$(document).ready(function () {
    table1 = $('#tableimported1').DataTable({
        ...
        "drawCallback": function (d) {
            ...
            if(isNaN(parseInt(d.json.Service_income))){
                $('.ServiceIncomePers').text(0);
                $('.RangeSelectorInput').val(0);
            }else{
...
function reload_tableimported1()
{
...
function CalcSerVal(ser, cur) {
    ...
    tb.find('tbody tr').each(function () {
        ...
        var w = parseInt($(this).find('td:eq(3)').text().replace(/,/g, '').replace(/\$|\€|\₹|\£/g, ''));
        var m = parseInt($(this).find('td:eq(4)').text().replace(/,/g, '').replace(/\$|\€|\₹|\£/g, ''));
        var q = parseInt($(this).find('td:eq(5)').text().replace(/,/g, '').replace(/\$|\€|\₹|\£/g, ''));
    ...
    if (isNaN(tw)) {
        $('.WTotal').text(cur + number_format(0, 0, ',', '.'));
    } else {
        $('.WTotal').text(cur + number_format(tw, 0, ',', '.'));
    }
    if (isNaN(tm)) {
        $('.MTotal').text(cur + number_format(0, 0, ',', '.'));
    } else {
        $('.MTotal').text(cur + number_format(tm, 0, ',', '.'));
    }
    if (isNaN(tq)) {
        $('.QTotal').text(cur + number_format(0, 0, ',', '.'));
    } else {
        $('.QTotal').text(cur + number_format(tq, 0, ',', '.'));
    }
    if (isNaN(ty1)) {
